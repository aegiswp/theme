 ...
 * Columns Block
 *
 * Modifies rendering and HTML output for the Columns block, including mobile stacking class logic.
 *
 * Responsibilities:
 * - Alters the front-end HTML output of the Columns block
 * - Applies or removes mobile stacking classes based on block attributes
 * - Integrates with utility functions for DOM and class manipulation
 *
 * Hooks/Filters Registered:
 * - 'render_block_core/columns': Modifies Columns block HTML on render
 *
 * @package    Aegis\Theme
 * @subpackage Blocks
 * @since      1.0.0
...
/**
 * Filter: 'render_block_core/columns'
 * Modifies Columns block HTML on render.
 *
 * @see render_columns_block()
 */
add_filter( 'render_block_core/columns', NS . 'render_columns_block', 10, 2 );
/**
 * Modifies the front-end HTML output of the Columns block.
 *
 * Alters the output for the 'core/columns' block, applying or removing mobile stacking classes based on block attributes.
 *
 * @since 1.0.0
 *
 * @param string $html  Block HTML.
 * @param array  $block Block data.
 * @return string Modified HTML for the Columns block.
 */
function render_columns_block( string $html, array $block ): string {
	...
	// Determine which stacking class to apply based on block attributes
	if ( $is_stacked && $block['attrs']['isStackedOnMobile'] === false ) {
		$class = 'is-not-stacked-on-mobile';
	}

	// Apply stacking class if needed
