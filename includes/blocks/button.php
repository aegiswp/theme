 ...
 * Button Block
 *
 * Modifies rendering and HTML output for the Button block, including custom styles and behaviors.
 *
 * Responsibilities:
 * - Alters the front-end HTML output of the Button block
 * - Applies custom styles and attributes based on block settings
 * - Integrates with global theme settings and utility functions
 *
 * Hooks/Filters Registered:
 * - 'render_block': Modifies Button block HTML on render
 *
 * @package    Aegis\Theme
 * @subpackage Blocks
 * @since      1.0.0
...
/**
 * Filter: 'render_block'
 * Modifies Button block HTML on render.
 *
 * @see render_button_block()
 */
add_filter( 'render_block', NS . 'render_button_block', 9, 2 );
/**
 * Modifies the front-end HTML output of the Button block.
 *
 * Alters the output for the 'core/button' block, applying custom styles, classes, and integrating with global settings.
 *
 * @since 1.0.0
 *
 * @param string $html  Block HTML.
 * @param array  $block Block data.
 * @return string Modified HTML for the Button block.
 */
function render_button_block( string $html, array $block ): string {
	$block_name = $block['blockName'] ?? null;

	// Only modify if this is the core/button block; use early priority for compatibility.
	if ( 'core/button' !== $block_name ) {
		return $html;
	}

	// Example: Modify output if the button uses the outline style
