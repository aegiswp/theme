...
$color_palette   = $global_settings['color']['palette']['theme'] ?? array();
$color_slugs     = wp_list_pluck( $color_palette, 'slug' );
$colors          = array();
$system_colors   = Color::SYSTEM_COLORS;

if ( ! function_exists( 'aegis_color_palette_grid' ) ) {
	/**
	 * Build the block markup for a grid of color shades.
	 *
	 * @param string $name   Color name (palette slug prefix).
	 * @param array  $shades Map of shade => hex value.
	 *
	 * @return string Generated block markup.
	 */
	function aegis_color_palette_grid( string $name, array $shades ): string {
		$count       = count( $shades );
		$columns     = $count >= 6 ? 6 : 3;
		$rows        = $count > 6 ? 2 : 1;
...
<div class="wp-block-group">
	...
	<?php foreach ( $colors as $color_name => $shades ) : ?>
		...
		<div class="wp-block-group has-14-font-size">
			...
			<?php echo aegis_color_palette_grid( $color_name, $shades ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Generated block markup; inner values are escaped. ?>
		</div>
		<!-- /wp:group -->
	<?php endforeach; ?>
	...
	<div class="wp-block-group has-14-font-size">
		...
		<?php echo aegis_color_palette_grid( 'system', $system ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Generated block markup; inner values are escaped. ?>
	</div>
	<!-- /wp:group -->
