...
class MetaBox implements Conditional, Styleable {
	 ...
	 * - `url`, `src`, `href` ΓÇö extracts the first image URL from an
	 *   `image_advanced` array, resolves attachment IDs, or passes
	 *   through a URL string
	 * - `alt` ΓÇö extracts alt text from an image array or attachment
	 * - `id` ΓÇö extracts the attachment ID as an integer
	 * - `content`, `text`, `value` ΓÇö implodes arrays into a
