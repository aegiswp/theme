...
// Enforces strict type checking for all code in this file, ensuring type safety for search block.
declare( strict_types=1 );

// Declares the namespace for the search block.
namespace Aegis\Framework\CoreBlocks;

// Imports classes, interfaces, and functions used by the search block.
use Aegis\Dom\CSS;
use Aegis\Dom\DOM;
...
class Search implements Renderable {
	...
	public function render( string $block_content, array $block, WP_Block $instance ): string {
		// Parse form structure and locate the search input.
		...
		// Read block attributes used for input and button styling.
		...
		// Collect existing input and button inline styles and classes.
		...
		// Apply background color when the button is inside or absent.
		...
		// Build custom box shadow from individual offset and blur values.
		if ( $box_shadow ) {
			$x      = $box_shadow['x'] ?? '0';
			$y      = $box_shadow['y'] ?? '0';
		...
		// Apply padding, border, and text color styles to the input.
		if ( $padding['top'] ?? '' ) {
		...
		// Override background when a dedicated input background is set.
		if ( $input_background ) {
		...
		// Write computed input styles and classes back to the DOM.
		if ( $input_styles ) {
		...
		// Apply margin to the form wrapper.
		...
		// Apply gap and border radius to the inner wrap element.
		...
		// Insert a search icon when the button is hidden or positioned outside.
		if ( $show_icon ) {
		...
		// Add a hidden post type field when a specific post type is configured.
