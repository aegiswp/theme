...
// Enforces strict type checking for all code in this file, ensuring type safety for button block.
declare( strict_types=1 );

// Declares the namespace for the button block.
namespace Aegis\Framework\CoreBlocks;

// Imports classes, interfaces, and functions used by the button block.
use Aegis\Dom\CSS;
use Aegis\Dom\DOM;
use Aegis\Dom\JS;
...
class Button implements Renderable {
	...
	public function render( string $block_content, array $block, WP_Block $instance ): string {
		...
		// Apply link attributes from block JSON (self-closing buttons store these in attrs only).
		$url = esc_url( $attrs['url'] ?? '' );
		if ( $url ) {
			$link->setAttribute( 'href', $url );
		} elseif ( ! $link->getAttribute( 'href' ) ) {
		...
		if ( ! empty( $attrs['linkTarget'] ) ) {
			$link->setAttribute( 'target', (string) $attrs['linkTarget'] );
		}

		if ( ! empty( $attrs['rel'] ) ) {
			$link->setAttribute( 'rel', (string) $attrs['rel'] );
		}
		...
		$this->apply_button_text( $dom, $link, $attrs );
	...
	/**
	 * Ensures button label text from block attributes is present in the link markup.
	 *
	 * Self-closing button comments store rich-text in attrs.text without inner HTML;
	 * Aegis icon rendering expects a text node to remain visible beside the SVG.
	 *
	 * @since 1.0.0
	 *
	 * @param \DOMDocument     $dom   Button markup document.
	 * @param \DOMElement      $link  Anchor or button element.
	 * @param array<string,mixed> $attrs Block attributes.
	 */
	private function apply_button_text( \DOMDocument $dom, \DOMElement $link, array $attrs ): void {
		$text = $this->get_button_text( $attrs );
		if ( '' === $text || $this->link_has_user_visible_text( $link ) ) {
			return;
		}
	 ...
	 * @param array<string,mixed> $attrs Block attributes.
	 */
	private function get_button_text( array $attrs ): string {
		$text = $attrs['text'] ?? '';
		if ( is_array( $text ) ) {
			return trim( wp_strip_all_tags( (string) ( $text['original'] ?? '' ) ) );
		}
	 ...
	 * @param \DOMElement $link Anchor or button element.
	 */
	private function link_has_user_visible_text( \DOMElement $link ): bool {
