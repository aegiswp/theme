...
final class TemplatePatternExpander {
	 ...
	 * @var bool
	 */
	private bool $is_rest_edit_context = false;
	 ...
	 * @return mixed
	 */
	public function capture_rest_edit_context( $result, $server, WP_REST_Request $request ) {
		$this->is_rest_edit_context = $request->get_param( 'context' ) === 'edit';
	 ...
	 * @return mixed
	 */
	public function reset_rest_edit_context( $response ) {
		$this->is_rest_edit_context = false;
	 ...
	 *
	 * @hook get_block_file_template 20
	 *
	 * @return WP_Block_Template|null
	 */
	public function expand_file_template( $template, string $id, string $template_type ) {
		...
		if ( ! did_action( 'init' ) || $this->is_rest_edit_context ) {
			return $template;
		}
	 ...
	 *
	 * @hook rest_request_after_callbacks 99
	 *
	 * @return mixed
	 */
	public function handle_rest_after_callbacks( $response, array $handler, WP_REST_Request $request ) {
		...
		if ( $context === 'edit' || $route === '/wp/v2/templates/lookup' ) {
			return $this->preserve_pattern_templates_for_edit( $response, $request, $route );
		}

		if ( $context === 'view' ) {
			return $this->expand_pattern_templates_in_response( $response );
		}
	 ...
	 */
	private function preserve_pattern_templates_for_edit( WP_REST_Response $response, WP_REST_Request $request, string $route ): WP_REST_Response {
		...
		if ( $this->is_indexed_template_list( $data ) ) {
			foreach ( $data as $index => $item ) {
				if ( ! is_array( $item ) ) {
					continue;
				}
				$type           = str_contains( $route, 'template-parts' ) ? 'wp_template_part' : 'wp_template';
				$data[ $index ] = $this->maybe_preserve_template_item( $item, $type, $route );
			}
		...
		$slug = $this->resolve_template_slug_from_route( $route, $request, $data );
		if ( $slug === '' ) {
			return $response;
		}

		$type = str_contains( $route, 'template-parts' ) ? 'wp_template_part' : 'wp_template';
		$response->set_data( $this->maybe_preserve_template_item( $data, $type, $route, $slug ) );
	 ...
	 */
	private function maybe_preserve_template_item( array $item, string $type, string $route, string $slug = '' ): array {
		$slug = $slug !== '' ? $slug : (string) ( $item['slug'] ?? '' );
		if ( $slug === '' ) {
			return $item;
		}

		$file = $this->read_theme_template_file( $slug, $type );
	 ...
	 * @param WP_REST_Response $response Response.
	 */
	private function expand_pattern_templates_in_response( WP_REST_Response $response ): WP_REST_Response {
		$data = $response->get_data();
	 ...
	 */
	private function is_indexed_template_list( array $data ): bool {
	 ...
	 */
	private function resolve_template_slug_from_route( string $route, WP_REST_Request $request, array $data ): string {
		if ( ! empty( $data['slug'] ) ) {
	 ...
	 * @param string $type Template type.
	 */
	private function read_theme_template_file( string $slug, string $type ): string {
