 ...
 */
function aeon_get_breadcrumb_trail( $args = array() ) {
	$breadcrumb = apply_filters( 'aeon_breadcrumb_trail_object', null, $args );

	if ( ! is_object( $breadcrumb ) ) {
		$breadcrumb = new Aeon_Breadcrumb_Trail( $args );
	}
...
/**
 * Creates a breadcrumbs menu for the site based on the current page that is being viewed by the user.
 *
 * @since  0.6.0
 * @access public
 */
class Aeon_Breadcrumb_Trail {
	 ...
	 * We will just return the trail HTML.
	 *
	 * @since  1.0.0
	 * @access public
	 * @return string
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'aeon_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );

		// Set the labels and post taxonomy properties.
		$this->set_labels();
		$this->set_post_taxonomy();

		// Let us find some items to add to the trail!
		$this->add_items();
	}
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="aeon-breadcrumb-trail" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
		...
		$breadcrumb = apply_filters( 'aeon_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'aeonwp' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'aeonwp' ),
			'home'                => esc_html__( 'Home',                                  'aeonwp' ),
			'error_404'           => esc_html__( '404 Not Found',                         'aeonwp' ),
			'archives'            => esc_html__( 'Archives',                              'aeonwp' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'aeonwp' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'aeonwp' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'aeonwp' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'aeonwp' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'aeonwp' ),
		...
		$this->labels = apply_filters( 'aeon_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'aeon_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'aeon_breadcrumb_trail_items', $this->items, $this->args ) );
	}
	...
	protected function add_singular_items() {
		...
		// If the post does not have a parent, get its hierarchy based off the post type.
		else {
	...
	protected function add_term_archive_items() {
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						if ( ! empty( $post_types ) ) {
							...
							// Core filter hook.
							$label = apply_filters( 'aeon_post_type_archive_title', $label, $post_type_object->name );
		...
		// If there is a single post type for the taxonomy, use it.
	...
	protected function add_post_type_archive_items() {
		...
		if ( false !== $post_type_object->rewrite ) {
			...
			// If there is a rewrite slug, check for parents.
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'aeonwp' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'aeonwp' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'aeonwp' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'aeonwp' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'aeonwp' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'aeonwp' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'aeonwp' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'aeonwp' ) ) );
	...
	protected function add_post_parents( $post_id ) {
		...
		while ( $post_id ) {
			...
			// If we hit a page that is set as the front page, bail.
			...
			// If there is no longer a post parent, break out of the loop.
			if ( 0 >= $post->post_parent ) {
				break;
			}
	...
	protected function add_post_hierarchy( $post_id ) {
		...
		elseif ( false !== $post_type_object->rewrite ) {
			...
			// If there is a path, check for parents.
			if ( ! empty( $post_type_object->rewrite['slug'] ) ) {
				$this->add_path_parents( $post_type_object->rewrite['slug'] );
			}
		}

		// If there is an archive page, add it to the trail.
		...
		// Map the rewrite tags if there is a `%` in the slug.
	...
	/**
	 * Gets post types by slug.  This is needed because the get_post_types() function does not exactly
	 ...
	 * hackish for now because we are checking for specific %tag% examples and only doing it for the 'post'
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'aeonwp' ) ) ) );
				}

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'aeonwp' ) ) ) );
				}

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'aeonwp' ) ) ) );
				}
