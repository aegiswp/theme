<?php
/**
 * Placeholder Image Extension for Aegis Theme
 *
 * This file handles the placeholder image functionality for the Aegis theme.
 * It provides the ability to display placeholder images when no image is selected,
 * enhancing the visual appearance of image blocks during content creation.
 *
 * The file includes functions to:
 * - Render placeholder images for empty image blocks
 * - Generate SVG placeholder icons when image files are not available
 * - Apply styling and attributes to placeholder elements
 *
 * @package aegis
 * @subpackage theme
 * @since 1.0.0
 ...
 *
 * Generates a placeholder image for empty image blocks, applying
 * appropriate styling and attributes based on block settings.
 * Supports both image files and SVG icons as placeholders.
 *
 * @since 1.0.0
 *
 ...
 *
 * @return string Modified block HTML with placeholder image.
 */
 ...
 *
 * Creates either an image element using placeholder image files
 * or an SVG icon when no placeholder images are available.
 * Cycles through available placeholder images for variety.
 *
 * @since 1.0.0
 *
 * @param DOMDocument $dom DOM document.
 *
 * @return ?DOMElement Placeholder image or icon element.
 */
function get_placeholder_image( DOMDocument $dom ): ?DOMElement {
	$image_paths = apply_filters(
		'aegis_placeholder_images',
		glob( get_dir() . 'assets/images/placeholder-*.webp' )
	);
