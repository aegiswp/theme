...
if ( ! class_exists( 'Aeon_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Aeon_WebFont_Loader {
		...
		public function __construct( $url = '' ) {
			...
			add_action( 'aeon_delete_fonts_folder', array( $this, 'aeon_delete_fonts_folder' ) );
		}
		...
		public function get_url() {
			...
			aeon_update_option( 'aeon_font_url', wp_json_encode( $font_url ) );
		...
		public function get_remote_url_contents() {
			...
			// Switch to a user-agent supporting woff2 if we do not need to support IE.
		...
		public function get_local_files_from_css() {
			...
			$stored     = get_site_option( 'aeon_downloaded_font_files', array() );
			...
			// If the fonts folder does not exist, create it.
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $url ) {
					...
					if ( file_exists( $font_path ) ) {
						...
						// Since the file exists we do not need to proceed with downloading it.
						continue;
					}
					 ...
					 */
					// require file.php if the download_url function does not exist.
			...
			if ( $change ) {
				...
				update_site_option( 'aeon_downloaded_font_files', $stored );
			}
		...
		public function preload_local_fonts() {
			...
			update_site_option( 'aeon_local_font_files', $local_font );
		...
		public function get_remote_files_from_css( $remote_styles = '' ) {
			...
			foreach ( $font_faces as $font_face ) {
				...
				// We are using array_flip here instead of array_unique for improved performance.
		...
		protected function write_stylesheet() {
			...
			// If the file does not exist, create it. Return false if it can not be created.
		...
		public function get_local_stylesheet_filename() {
			return apply_filters( 'aeon_local_font_file_name', 'aeon-fonts' );
		}
		...
		public function set_font_format( $format = 'woff2' ) {
			$this->font_format = apply_filters( 'aeon_local_google_fonts_format', $format );
		}
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'aeon_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'aeon_local_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'aeon_local_fonts_directory_name', 'aeon' );
			}
		...
		public function schedule_cleanup() {
			if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
				if ( ! wp_next_scheduled( 'aeon_delete_fonts_folder' ) && ! wp_installing() ) {
					wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'aeon_delete_fonts_folder' ); // phpcs:ignore WPThemeReview.PluginTerritory.ForbiddenFunctions.cron_functionality_wp_schedule_event
				}
			}
		}
		...
		public function delete_fonts_folder() {
			...
			aeon_delete_option( 'aeon_font_url' );
			delete_site_option( 'aeon_local_font_files' );
 ...
 * @return object
 */
function aeon_webfont_instance( $font_url = '' ) {
	return new Aeon_WebFont_Loader( $font_url );
}
 ...
 */
function aeon_get_webfont_url( $url, $format = 'woff2' ) {

	// Check if the Google font URL is already present or not.
	$font_url = aeon_get_option( 'aeon_font_url', false );
	if ( $font_url ) {
		return json_decode( $font_url );
	}

	// Now create the Google font URL if its not present.
	$font = aeon_webfont_instance( $url );
 ...
 * @param string $format The font-format. If you need to support IE, change this to "woff".
 */
function aeon_preload_fonts( $url, $format = 'woff2' ) {

	// Check if cached font files data preset present or not.
	$font_files = get_site_option( 'aeon_local_font_files', false );

	if ( is_array( $font_files ) && ! empty( $font_files ) ) {
		$font_format = apply_filters( 'aeon_local_google_fonts_format', $format );
	...
	// Now preload font data after processing it, as we did not get stored data.
	$font = aeon_webfont_instance( $url );
	$font->set_font_format( $format );
 ...
 * @return void
 */
function aeon_regenerate_fonts_folder() {

	check_ajax_referer( 'aeon-regenerate-local-fonts', 'nonce' );

	if ( ! current_user_can( 'edit_theme_options' ) ) {
		wp_send_json_error( 'invalid_permissions' );
	}

	if ( aeon_get_option( 'load_fonts_locally' ) ) {
		$loader = aeon_webfont_instance( '' );
...
add_action( 'wp_ajax_aeon_regenerate_fonts_folder', 'aeon_regenerate_fonts_folder' );

/**
 * Delete the cached partial configs.
 */
function aeon_delete_cached_partials() {
	delete_option( 'aeon_partials_config_cache' );
...
add_action( 'customize_save_after', 'aeon_delete_cached_partials' );
