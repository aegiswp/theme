...
( function () {
	...
	function getTimeRemaining(
		...
		if ( timezone === 'local' ) {
			...
			const utcString = datetime.endsWith( 'Z' )
				? datetime
				: datetime + 'Z';
	...
	function initCountdown( wrapper: HTMLElement ): void {
		...
		const expiredContainer = wrapper.querySelector< HTMLElement >(
			'.aegis-countdown__expired'
		);
		const srRegion = wrapper.querySelector< HTMLElement >(
			'.aegis-countdown__sr'
		);
		...
		units.forEach( ( unit ) => {
			...
			if ( segment ) {
				digitElements[ unit ] = segment.querySelector< HTMLElement >(
					'.aegis-countdown__digits'
				);
			}
		} );
