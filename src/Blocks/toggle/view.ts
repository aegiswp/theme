	...
	function parseConfig( el: HTMLElement ): ToggleConfig {
		return {
			...
			animationDuration: parseInt(
				el.getAttribute( 'data-animation-duration' ) || '300',
				10
			),
		};
	}
	...
	function initToggle( wrapper: HTMLElement ): void {
		...
		function toggle( open?: boolean ): void {
			...
			if ( shouldOpen ) {
				...
				const body = contentEl.querySelector< HTMLElement >(
					'.aegis-toggle__body'
				);
				const height = body
					? body.scrollHeight
			...
			if ( ! config.allowMultiple ) {
				...
				if ( parent ) {
					...
					siblings.forEach( ( sibling ) => {
						if (
							sibling !== wrapper &&
							sibling.classList.contains( 'aegis-toggle--open' )
						) {
							const siblingTrigger =
								sibling.querySelector< HTMLButtonElement >(
									'.aegis-toggle__trigger'
								);
							if ( siblingTrigger ) {
